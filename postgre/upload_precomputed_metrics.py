...
def read_precomputed_metrics(path: Path):
    rows = []
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Upload raw precomputed_metrics data into PostgreSQL with fixed headers."
    )
    parser.add_argument(
        "path",
        nargs="?",
        default=r"C:\\Users\\Admin\\OneDrive\\Documents\\precomputed_metrics",
        help="Path to the precomputed_metrics file.",
    )
    parser.add_argument(
        "--host",
...
def main():
    ...
    rows = read_precomputed_metrics(path)
    if not rows:
        raise ValueError(f"No rows found in precomputed_metrics file: {path}")
    ...
    try:
        ...
        print(f"Uploaded precomputed_metrics into {args.database}.{args.schema}.{args.table}")
    finally:
        connection.close()
