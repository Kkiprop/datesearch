...
DEFAULT_JSON = r"C:\Users\Admin\Downloads\cost_price.json"
...
def create_table(cur):
    ...
    cur.execute("DROP TABLE IF EXISTS cost_price;")

    cur.execute(
        """
        CREATE TABLE cost_price (
            date DATE PRIMARY KEY,
            store_counts JSONB NOT NULL,
            total_barcode_count INTEGER,
...
def upsert(cur, document):
    ...
    cur.execute(
        ...
        ON CONFLICT (date)
        DO UPDATE SET
            store_counts = EXCLUDED.store_counts,
            total_barcode_count = EXCLUDED.total_barcode_count,
            updated_at = EXCLUDED.updated_at;
        """,
        (
            document.get("date"),
            Json(document.get("store_counts", {})),
            document.get("total_barcode_count"),
            updated_at,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload Cost Price data to PostgreSQL"
    )
