...
def read_payroll_timecard_entries(path: Path):
    rows = []
...
def copy_rows(connection, schema, table_name, columns, rows):
    ...
    with io.StringIO() as buffer:
        ...
        for fields in rows:
            padded = [fields[i] if i < len(fields) else None for i in range(len(columns))]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Upload payroll_timecard_entries into PostgreSQL with generic columns.",
    )
    parser.add_argument(
        "path",
        nargs="?",
        default=r"C:\\Users\\Admin\\OneDrive\\Documents\\payroll_timecard_entries",
        help="Path to the payroll_timecard_entries file.",
    )
    parser.add_argument(
        "--host",
    ...
    parser.add_argument(
        "--table", "-t",
        default="payroll_timecard_entries",
...
def main():
    ...
    rows = read_payroll_timecard_entries(path)
    if not rows:
        raise ValueError(f"No rows found in payroll_timecard_entries file: {path}")
    ...
    try:
        ...
        print(f"Uploaded payroll_timecard_entries into {args.database}.{args.schema}.{args.table}")
    finally:
        connection.close()
