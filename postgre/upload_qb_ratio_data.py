...
def read_qb_ratio_data(path: Path):
    rows = []
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Upload raw qb_ratio_data into PostgreSQL with fixed headers."
    )
    parser.add_argument(
        "path",
        nargs="?",
        default=r"C:\\Users\\Admin\\OneDrive\\Documents\\qb_ratio_data",
        help="Path to the qb_ratio_data file.",
    )
    parser.add_argument(
        "--host",
...
def main():
    ...
    rows = read_qb_ratio_data(path)
    if not rows:
        raise ValueError(f"No rows found in qb_ratio_data file: {path}")
    ...
    try:
        ...
        print(f"Uploaded qb_ratio_data into {args.database}.{args.schema}.{args.table}")
    finally:
        connection.close()
