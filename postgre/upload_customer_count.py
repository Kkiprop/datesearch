...
DEFAULT_JSON = r"C:\Users\Admin\Downloads\customer_count.json"
...
def create_table(cur):
    cur.execute("DROP TABLE IF EXISTS customer_count;")

    cur.execute(
        """
        CREATE TABLE customer_count (
            date DATE PRIMARY KEY,
            customer_count JSONB NOT NULL,
...
def upsert(cur, document):
    ...
    cur.execute(
        ...
        (
            document.get("date"),
            Json(document.get("customer_count", {})),
            updated_at,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Upload Customer Count data to PostgreSQL"
    )
