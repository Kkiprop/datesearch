...
DEFAULT_JSON = r"C:\Users\Admin\Downloads\Dashboard_data.waste_tool.json"
...
def create_table(cur):
    cur.execute("DROP TABLE IF EXISTS waste_tool;")

    cur.execute("""
        CREATE TABLE waste_tool (
            date DATE PRIMARY KEY,
            total_waste_per_store JSONB NOT NULL,
...
def upsert(cur, document):
    ...
    cur.execute(
        """
        INSERT INTO waste_tool (
            date,
            total_waste_per_store,
            updated_at
        )
        VALUES (%s, %s, %s)
        ...
        ON CONFLICT (date)
        DO UPDATE SET
            total_waste_per_store = EXCLUDED.total_waste_per_store,
            updated_at = EXCLUDED.updated_at;
        """,
        (
            document.get("date"),
            Json(document.get("total_waste_per_store", {})),
            updated_at,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import Waste Tool data into PostgreSQL"
    )
